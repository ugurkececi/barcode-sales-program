    ...
    public partial class fSatis : Form
    {
        ...
        private void HizliButonDoldur()
        {
            ...
            foreach (var item in hizliurun)
            {
                ...
                if (bH != null)
                {
                    ...
                    double fiyat = islemler1.DoubleYap(item.Fiyat.ToString());
                    bH.Text = item.UrunAd + "\n" + fiyat.ToString("C2");
        ...
        private void UrunGetirListeye(Urun urun, string barkod, double miktar)
        {
            ...
            if (!eklenmismi)
            {
                ...
                gridSatisListesi.Rows[satirsayisi].Cells["UrunGrup"].Value = urun.Birim;
        ...
        private void bOdenen_Click(object sender, EventArgs e)
        {
                if (tNumarator.Text != "")
            {
                double sonuc = islemler1.DoubleYap(tNumarator.Text) - islemler1.DoubleYap(tGenelToplam.Text);
                tParaUstu.Text = sonuc.ToString("C2");
                tOdenen.Text = islemler1.DoubleYap(tNumarator.Text).ToString("C2");
                tNumarator.Clear();
                tBarkod.Focus();
        ...
        {
            Button b = (Button)sender;
            double sonuc = islemler1.DoubleYap(b.Text) - islemler1.DoubleYap(tGenelToplam.Text);
            tOdenen.Text = islemler1.DoubleYap(b.Text).ToString("C2");
            tParaUstu.Text = sonuc.ToString("C2");
               ...
               for(int i= 0; i< satirsayisi; i++)
                {
                    ...
                    satis.AlisFiyat = islemler1.DoubleYap(gridSatisListesi.Rows[i].Cells["AlisFiyati"].Value.ToString());
                    satis.SatisFiyat = islemler1.DoubleYap(gridSatisListesi.Rows[i].Cells["Fiyat"].Value.ToString());
                    satis.Miktar = islemler1.DoubleYap(gridSatisListesi.Rows[i].Cells["Miktar"].Value.ToString());
                    satis.Toplam = islemler1.DoubleYap(gridSatisListesi.Rows[i].Cells["Toplam"].Value.ToString());
                    satis.KdvTutari =islemler1.DoubleYap(gridSatisListesi.Rows[i].Cells["Toplam"].Value.ToString());
                    ...
                    if(!satisiade)
                    {
                        islemler1.StokAzalt (satis.Barkod = gridSatisListesi.Rows[i].Cells["Barkod"].Value.ToString(), islemler1.DoubleYap(gridSatisListesi.Rows[i].Cells["Miktar"].Value.ToString()));
                    }
                    else
                    {
                        islemler1.StokArtir(satis.Barkod = gridSatisListesi.Rows[i].Cells["Barkod"].Value.ToString(), islemler1.DoubleYap(gridSatisListesi.Rows[i].Cells["Miktar"].Value.ToString()));
                    }
                    alisfiyattoplam += islemler1.DoubleYap(gridSatisListesi.Rows[i].Cells["AlisFiyati"].Value.ToString());
                ...
                {
                    case "Nakit":
                        io.Nakit = islemler1.DoubleYap(tGenelToplam.Text);
                        io.Kart = 0; break;
                    case "Kart":
                        io.Nakit = 0;
                        io.Kart = islemler1.DoubleYap(tGenelToplam.Text); break;
                    case "Kart-Nakit":
                        io.Nakit = islemler1.DoubleYap(lNakit.Text);
                        io.Kart = islemler1.DoubleYap(lKart.Text); break;

                }
                db.IslemOzet.Add(io);
                db.SaveChanges();

                var islemonartir = db.Islem.First();
                islemonartir.IslemNo += 1;
                db.SaveChanges();
